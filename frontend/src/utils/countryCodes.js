// Comprehensive world country dialing codes (ISO 3166-1 standard)
...
// Pinned popular countries for quick access at top
...
export const ALL_COUNTRIES = [
  ...
  { name: 'British Virgin Islands', code: '+1284', iso: 'vg', flag: '🇻🇬' },
  ...
  { name: 'Turks and Caicos', code: '+1649', iso: 'tc', flag: '🇹🇨' },
...
// Sort helper by code length descending to ensure specific prefixes match first (e.g. +1868 before +1)
const SORTED_BY_CODE_LEN = [...ALL_COUNTRIES].sort((a, b) => b.code.length - a.code.length);

/**
 * Finds country by dial code or returns default India
 */
export function getCountryByCode(code) {
  if (!code) return DEFAULT_COUNTRY;
  const cleanCode = code.startsWith('+') ? code : `+${code}`;
  return ALL_COUNTRIES.find((c) => c.code === cleanCode) || DEFAULT_COUNTRY;
}
...
export function getCountryByIso(iso) {
  ...
  return ALL_COUNTRIES.find((c) => c.iso.toLowerCase() === iso.toLowerCase()) || DEFAULT_COUNTRY;
}

/**
 * Safely parse a full international phone string into { countryCode, number, country }
 */
export function parsePhone(phoneString) {
  const phone = (phoneString || '').trim();
  if (!phone) {
    return { countryCode: '+91', number: '', country: DEFAULT_COUNTRY };
  }

  // Check if string starts with +
  if (phone.startsWith('+')) {
    const matched = SORTED_BY_CODE_LEN.find((c) => phone.startsWith(c.code));
    if (matched) {
      return {
        countryCode: matched.code,
        number: phone.slice(matched.code.length).replace(/\D/g, ''),
        country: matched,
      };
    }
  }

  // Fallback: if just 10 digits without +, default to India
