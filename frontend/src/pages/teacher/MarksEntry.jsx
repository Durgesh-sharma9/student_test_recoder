...
export default function MarksEntry() {
  ...
  return (
    <div className="space-y-4">
      ...
      <Card><CardHeader><CardTitle>Create Monthly Result Session</CardTitle></CardHeader><CardContent className="grid md:grid-cols-3 gap-2">
        ...
        <Input placeholder="Month (1-12)" value={form.month} onChange={(e) => setForm({ ...form, month: e.target.value })} />
        ...
        <Button onClick={createSession}>Load Marks Table</Button>
      </CardContent></Card>

      {session && (
        <Card><CardHeader><CardTitle>Editable Marks Table</CardTitle></CardHeader><CardContent className="space-y-3">
          <div className="overflow-x-auto">
            <Table><TableHeader><TableRow><TableHead>Roll No</TableHead><TableHead>Name</TableHead><TableHead>Marks</TableHead><TableHead>Rank</TableHead><TableHead>%</TableHead></TableRow></TableHeader>
              <TableBody>
                {rows.map((r, idx) => (
                  <TableRow key={r.studentId}>
                    <TableCell>{r.rollNo}</TableCell>
                    <TableCell>{r.name}</TableCell>
                    <TableCell><Input type="number" min="0" max={form.maxMarks} value={r.marksObtained} onChange={(e) => setRows((prev) => prev.map((x, i) => i === idx ? { ...x, marksObtained: e.target.value } : x))} /></TableCell>
          ...
          <div className="flex gap-2">
            ...
            <Button variant="outline" onClick={download}>Download Entered Data</Button>
          </div>
        </CardContent></Card>
      )}
    </div>
