...
export const sendDailyTestPdf = (res, exportData) => {
  ...
  doc.font('Helvetica-Bold').fontSize(14).text(meta.schoolName, { align: 'center' });
  doc.moveDown(0.4);
  ...
  if (mode === 'single') {
    doc.text(`Test Date: ${meta.testDate}`, { align: 'center' });
  } else {
    doc.text(`Date Range: ${meta.dateFrom} to ${meta.dateTo}`, { align: 'center' });
  }
  doc.moveDown(0.6);
  doc.font('Helvetica-Bold').fontSize(13).text('DAILY TEST REPORT', { align: 'center' });
  doc.moveDown(0.8);
  ...
  doc.text(`Total Students: ${meta.totalStudents}`, 36, footerY, { width: contentWidth / 2, align: 'left' });
